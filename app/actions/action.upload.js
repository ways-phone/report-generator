...
export const readFile = path => dispatch => {
  if (!path.match(/\.csv$/)) {
    return dispatch(readFileError(new Error('Incompatible File Type. File must be of type csv')));
  }
  let parser;
  ...
  const config = { name: parser.name, path: parser.path, data, isCPH: parser.isCPHFile() };
